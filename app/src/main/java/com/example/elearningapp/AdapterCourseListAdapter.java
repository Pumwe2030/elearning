...
public class AdapterCourseListAdapter extends RecyclerView.Adapter<AdapterCourseListAdapter.viewHolder> {
    ...
    public void onBindViewHolder(@NonNull AdapterCourseListAdapter.viewHolder holder, int position) {
        ...
        holder.iv_delete.setOnClickListener(v -> {
            ...
            builder.setTitle("Delete")
                    ...
                    .setPositiveButton("Delete", (dialog, which) -> deleteVideo(modelVideo))
    ...
    private void deleteVideo(ModelCourse modelVideo) {
        ...
        databaseReference.child(modelVideo.title)
                .removeValue()
                .addOnSuccessListener(unused -> Toast.makeText(context, "Course deleted successfully", Toast.LENGTH_SHORT).show()).addOnFailureListener(e -> Toast.makeText(context, e.getMessage(), Toast.LENGTH_SHORT).show());
